...
import pandas as pd
import plotly.express as px
# from constants import update_output
import json
...
sidebar = html.Div(
    [
        html.Img(src=app.get_asset_url('logo1.png'), style={'height':'40%', 'textAlign': 'center', 'marginLeft': '10%'}),
        dbc.Nav(
            [
                dbc.NavLink("Panduan", href="/", active="exact"),
                dbc.NavLink("Simulasi", href="/page-1", active="exact"),
                #dbc.NavLink("Simulasi", href="/page-2", active="exact"),
            ],
            vertical=True,
            pills=True,
        ),
...
page_1_layout = html.Div([
                ...
                dbc.Row([dbc.Col(html.Div(
                    dbc.Button("SUBMIT", color="info", size="lg", className="me-1",id='btn-submit')
                ))
            ]
        ),
        dcc.Store(id='data'),
        dbc.Spinner([dcc.Graph(id='grafik7'),dcc.Graph(id='grafik1'), dcc.Graph(id='grafik8'),dcc.Graph(id='grafik2'),dcc.Graph(id='grafik3'), dcc.Graph(id='grafik4'),dcc.Graph(id='grafik5'),dcc.Graph(id='grafik6')], spinner_style={"width": "5rem", "height": "5rem"}, color="primary")
                ] )
            
# page_2_layout = html.Div([
#         dcc.Store(id='data'),
#         dcc.Graph(id='grafik1'),
#         dcc.Graph(id='grafik2'),
#         dcc.Graph(id='grafik3'),
#         dcc.Graph(id='grafik4'),
#         dcc.Graph(id='grafik5'),
#         dcc.Graph(id='grafik6')])
...
def update_output(clicked, input1, input2, input3, input4, input5, input6, input7, input8, input9, input10, input11, input12, input13, input14, input15, input16, input17):
    ...
    if clicked:
        ...
        eth_prices = json.load(open('etherium1.json'))
